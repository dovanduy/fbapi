...
let uploadPost = ({privacy,cookie,agent,content,color,photoID,youtube,location,fb_dtsg})=>{
    return new Promise(resolve => {
        let option = {
            method:'post',
            ...
            form:{
                "variables": JSON.stringify(data(location,id(cookie,"c_user=",";"),content,color,photoID,youtube)),
                "__user":id(cookie,"c_user=",";"),
                "__a":"1",
                "fb_dtsg":fb_dtsg
...
let run = async ({location,cookie,agent,content,ImageArr,color,youtubeLink})=>{
    ...
    let upload = await uploadPost({privacy:privacy,cookie:cookie,agent:agent,content:content,color:color,photoID:photoID,youtube:youtube,location:location,fb_dtsg:fb_dtsg});


    if(upload.error === '' && isNaN(upload.postID) === true && upload.type === 'group'){

        upload = await run({location,cookie,agent,content,ImageArr,color,youtubeLink});
    }
